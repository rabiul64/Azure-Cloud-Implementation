...
namespace TimeSeriesSequence
{
    public class MultiSequenceTaxiPassanger
    {
        ...
        public double ExpAccuracy { get; set; }
        public string OutputPath { get; set; }
        public string PredictionOutputPath { get; set; }
        ...
        /// </summary>
        public void RunPassangerTimeSeriesSequenceExperiment( string dataset, string PredicationDateTime)
        {
            int inputBits = 88;
            int maxCycles = 25;
            int numColumns = 1024;
            ...
            var taxiData = CSVPRocessingMethods.ProcessExistingDatafromCSVfile(dataset);
            ...
            PredictPassanagerWithUserInput(learningExpResult, PredicationDateTime);
        }
        ...
        private void PredictPassanagerWithUserInput(HtmPredictionEngine learningExpResult, string PredicationDateTime)
        {
            ...
            string[] dateList = PredicationDateTime.Split(",");
            foreach (string userInputDateTime in dateList)
            {
                RunPassangerPrediction(userInputDateTime, learningExpResult);
            }

            string filename = "Result" + DateTime.Now.Ticks.ToString() + ".txt";
           // string BasePath = AppDomain.CurrentDomain.BaseDirectory;
           // string localFilePathPrediction = Path.Combine(BasePath, filename);

            string localFilePathPrediction = filename;

            using (StreamWriter swOutput = File.CreateText(localFilePathPrediction))
            {
                swOutput.WriteLine($"{filename}");
                foreach (var predictions in UserPredictedValues)
                {
                    swOutput.WriteLine("******User Prediction Result Starting*****");
                    foreach (var prediction in predictions)
                    {
                        swOutput.WriteLine(prediction);
                    }
                    swOutput.WriteLine("****User Prediction Result Ending*****");

                }
            }

            PredictionOutputPath = localFilePathPrediction;
        }
        ...
        private void RunPassangerPrediction(string userInputDateTime, HtmPredictionEngine learningExpResult)
        {
            ...
            data.Add("User DateTime Input : ", " " + userInputDateTime);
                try
                {
                    ...
                    if (predictedValuesForUserInput.Count > 0)
                    {
                        foreach (var predictedVal in predictedValuesForUserInput)
                        {
                            ...
                            data.Add("PASSANGER SIMILARITY " + predictedVal.Similarity, " PREDICTED PASSANGER NO :" + predictedVal.PredictedInput);
                        }
                    }
                    else {
                        data.Add("PASSANGER SIMILARITY :" + " 0", " PREDICTED PASSANGER NO :" + " 0");
                    }

                    UserPredictedValues.Add(data);
                }
        ...
        /// <returns></returns>
        private HtmPredictionEngine RunExperiment(int inputBits, int maxCycles, int numColumns, EncoderBase encoder, List<Dictionary<string, int[]>> trainTaxiData)
        {
            ...
            filename = "PredictionExperimentLearning" + now.Ticks.ToString() + ".txt";
            ...
            using (StreamWriter swOutput = File.CreateText(localFilePath))
            {
                swOutput.WriteLine($"{filename}");
